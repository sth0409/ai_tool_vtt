...
const HVC_LOW_VALUE_PHRASES: RegExp[] = [
...
function isHighValueHvcTerm(term: string): boolean {
  ...
  if (HVC_LOW_VALUE_PHRASES.some((re) => re.test(phrase))) return false;

  if (words.length === 1) {
    const w = words[0];
    if (w.length <= 3) return false;
    if (HVC_LOW_VALUE_WORDS.has(w)) return false;
    return true;
  }

  const lowValueCount = words.filter((w) => HVC_LOW_VALUE_WORDS.has(w)).length;
  if (lowValueCount >= words.length - 1) return false;
  return true;
}
...
function toAiResultStringList(value: unknown, predicate?: (item: string) => boolean): string[] {
